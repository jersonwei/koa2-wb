...
;(function (root, factory) {
  ...
  App = (function () {
    ...
    App.prototype.createContainer = function (doc) {
      ...
      return $(doc.body).append(
        (this.$el = $("<div class='atwho-container'></div>"))
      )
    }
    ...
    App.prototype.reg = function (flag, setting) {
      ...
      controller =
        ...
        (base[flag] = this.$inputor.is('[contentEditable]')
  ...
  Controller = (function () {
    ...
    function Controller (app, at1) {
      ...
      this.range = null
      if (
        (this.$el = $('#atwho-ground-' + this.id, this.app.$el)).length === 0
      ) {
        this.app.$el.append(
    ...
    Controller.prototype._lookUp = function (query) {
      ...
      return this.model.query(
        query.text,
        $.proxy(_callback, this, this.expectedQueryCBId)
      )
    }
  ...
  EditableController = (function (superClass) {
    ...
    EditableController.prototype.catchQuery = function (e) {
      ...
      if (/firefox/i.test(navigator.userAgent)) {
        ...
        if (
          ...
          if (
            $(_range.cloneContents())
              .contents()
              .last()
              .is('.atwho-inserted')
          ) {
            inserted = $(range.startContainer)
              .contents()
              .get(offset)
            this._setRange(
              'after',
      ...
      if (
        ($query = $('.atwho-query', this.app.document)).length > 0 &&
        $query.is(':empty') &&
        $query.text().length === 0
      ) {
        $query.remove()
      ...
      if (
        $query.length > 0 &&
        (query_content = $query.attr('data-atwho-at-query'))
      ) {
        $query
          .empty()
          .html(query_content)
      ...
      matched = this.callbacks('matcher').call(
        this,
        this.at,
        _range.toString(),
        this.getOpt('startWithSpace'),
      ...
      if (
        $query.length === 0 &&
        isString &&
        (index = range.startOffset - this.at.length - matched.length) >= 0
      ) {
        range.setStart(range.startContainer, index)
        $query = $('<span/>', this.app.document)
          .attr(this.getOpt('editableAtwhoQueryAttrs'))
          .addClass('atwho-query')
      ...
      if (isString && matched.length <= this.getOpt('maxLen', 20)) {
        ...
        if ($query.text().indexOf(this.at) >= 0) {
          if (this._movingEvent(e) && $query.hasClass('atwho-inserted')) {
            ...
            false !==
            this.callbacks('afterMatchFailed').call(this, this.at, $query)
          ) {
            this._setRange(
              'after',
              this._unwrap(
    ...
    EditableController.prototype.insert = function (content, $li) {
      ...
      suffix =
        (suffix = this.getOpt('suffix')) === '' ? suffix : suffix || '\u00A0'
      ...
      if ((range = this._getRange())) {
        ...
        range.collapse(false)
        range.insertNode(
          (suffixNode = this.app.document.createTextNode('' + suffix))
        )
  ...
  Model = (function () {
    ...
    Model.prototype.save = function (data) {
      return this.storage.data(
        this.at,
        this.context.callbacks('beforeSave').call(this.context, data || [])
      )
    }
  ...
  View = (function () {
    function View (context) {
      this.context = context
      this.$el = $(
        "<div class='atwho-view'><ul class='atwho-view-ul'></ul></div>"
      )
    ...
    View.prototype.reposition = function (rect) {
      ...
      if (
        rect.bottom + this.$el.height() - $(_window).scrollTop() >
        $(_window).height()
      ) {
        rect.bottom = rect.top - this.$el.height()
      }
      if (
        rect.left > (overflowOffset = $(_window).width() - this.$el.width() - 5)
      ) {
    ...
    View.prototype.prev = function () {
      ...
      offset =
        prevEl.offsetTop +
        prevEl.offsetHeight +
        (prevEl.nextSibling ? prevEl.nextSibling.offsetHeight : 0)
      return this.scrollTop(Math.max(0, offset - this.$el.height()))
    }
    ...
    View.prototype.render = function (list) {
      ...
      for (i = 0, len = list.length; i < len; i++) {
        ...
        li = this.context
          .callbacks('tplEval')
          .call(this.context, tpl, item, 'onDisplay')
        $li = $(
          this.context
  ...
  $.fn.atwho = function (method) {
    ...
    this.filter(
      ...
      if (typeof method === 'object' || !method) {
        ...
        return (result = Api[method].apply(
          app,
